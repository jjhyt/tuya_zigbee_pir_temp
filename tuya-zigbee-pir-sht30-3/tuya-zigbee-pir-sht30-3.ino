//这个程序跟第二版的区别就是有人体感应的时候才上报温湿度,平时不再固定20分钟上报一次,二版的程序功耗2032电池只能坚持三天!
//怀疑跟程序频繁唤醒有关!
...
int shu = 0;          //看门狗唤醒记数
int wakeby = 0;           //记录哪个中断唤醒 1=PIR,2=按钮,3=看门狗
...
void reportPir(void)
{
  ...
  if (abs(lightLevel - lastlux) > 10) {                             //光线波动大于10,才上报
    lastlux = lightLevel;
...
void dianliang(void)           //调试使用,观察狗唤醒记数
{
  digitalWrite(TY_PIN, LOW);   //先唤醒模组
  delay(1);
  digitalWrite(TY_PIN, HIGH);   //拉高唤醒脚
  
  mcu_dp_value_update(DPID_BATTERY_PERCENTAGE, long(shu)); 
  delay(50);
}
